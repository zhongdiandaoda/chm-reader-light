...
```
  ...
  The preflight checks the required macOS build tools before tests or packaging begin. Packaging reuses an existing CHMLib build only after its source, patches, provenance, hashes, deployment target, and architecture pass the full verifier. When a rebuild is needed, it reuses a cached source archive only after its pinned SHA-256 passes, otherwise it downloads the pinned CHMLib commit. It then applies the repository's CVE-2025-48172 and extraction-budget patches, builds arm64 Mach-O binaries, and verifies the staged provenance. The generated macOS icon is also cached by the hashes of its source artwork, generator scripts, parameters, and tool versions; the cached ICNS hash is verified before reuse.
