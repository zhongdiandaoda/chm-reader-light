...
- `npm run check:release-bundle:mac -- <release.zip> <arm64|x64>` on macOS to inspect the final archive's signature, native architectures, bundled CHMLib link, bundle identifier, exact generated project icon, and allowlisted runtime ASAR contents and size.
...
- `npm run prepare:homebrew-cask -- --release-dir <release-dir>` after staging and verifying release artifacts to generate a copy-ready cask draft from the published zip checksums; do not announce Homebrew support until the cask is published and verified.
